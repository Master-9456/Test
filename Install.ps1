...
if ($win11 -or $win10 -or $win8_1 -or $win8) {
    ...
    if (Get-AppxPackage -Name SpotifyAB.SpotifyMusic) {
        Write-Host @'
Обнаружена версия Spotify Microsoft Store, которая не поддерживается.
'@`n
        $ch = Read-Host -Prompt "Удалить Spotify Microsoft Store ? (Y/N) "
        if ($ch -eq 'y') {
            Write-Host @'
Удаление Spotify.
'@`n
...
Write-Host 'Скачивание последнего патча BTS...'`n
...
if (-not $spotifyInstalled) {
    ...
    Write-Host "Скачивание и установка Spotify " -NoNewline
...
# Удаление временных установщиков 
...
$ch = Read-Host -Prompt "Хотите заблокировать автоматические обновления Spotify ? (Y/N), Разблокировать снова (U)"
if ($ch -eq 'y') {
    ...
    if ($Check_folder_file -match '\bSystem\b' -and $Check_folder_file -match '\bReadOnly\b' -and $migrator_bak ) {
        ...
        $ch = Read-Host -Prompt 'Ого, обновления уже заблокированы, хотите удалить блокировку ? (Y/N)'
...
$ch = Read-Host -Prompt "Хотите уставновить автоматическую очистку кэша ? (Y/N) Удалить скрипт (U)"
if ($ch -eq 'y') {
    ...
    If ($test_cache_spotify_ps -and $test_spotify_vbs) {
        $ch = Read-Host -Prompt "Очистка кэша уже установлена, хотите удалить ? (Y/N)"
        if ($ch -eq 'y') {
            ...
            $Shortcut.TargetPath = $source
            $Shortcut.Save()
            Write-Host "Скрипт очистки кеша удален" -ForegroundColor Green
            Write-Host "Установка завершена" -ForegroundColor Green
            exit
        }
        if ($ch -eq 'n') {
    ...
    add-content -Path $env:APPDATA\Spotify\cache-spotify.ps1 -Value '# Запускаем Spotify.exe и ждем пока процесс будет остановлен' -passthru | Out-Null
    ...
    add-content -Path $env:APPDATA\Spotify\cache-spotify.ps1 -Value '# Это основной блок который удаляет файлы кэша по времени последнего доступа, файлы которые не были изменены и не открывались более указанного вами количества дней, будут удалены. (кол-во дней = seven)' -passthru | Out-Null
    ...
    add-content -Path $env:APPDATA\Spotify\cache-spotify.ps1 -Value '}' -passthru | Out-Null
    add-content -Path $env:APPDATA\Spotify\cache-spotify.ps1 -Value '' -passthru | Out-Null
    add-content -Path $env:APPDATA\Spotify\cache-spotify.ps1 -Value '# Удаляем файл mercury.db если его размер превышает 100 MB.' -passthru | Out-Null
    ...
    if ($ch -match "^[1-9][0-9]?$|^100$") {
        ...
        Write-Host "Очистка кэша успешно установлена" -ForegroundColor Green
        Write-Host "Установка завершена" -ForegroundColor Green
        exit
    }
    if (!($ch -match "^[1-9][0-9]?$|^100$")) {
        $ch = Read-Host -Prompt "Ой, неудачно, пробуем снова
    ...
    Введите количество дней от 1 до 100"
        ...
        Write-Host 'Пожалуйста, откройте файл cache-spotify.ps1 по этому пути "AppData\Roaming\Spotify" и введите своё кол-во дней'
        Write-Host "Установка завершена" -ForegroundColor Green
        exit
    }
...
if ($ch -eq 'u') {
    ...
    If ($test_cache_spotify_ps -and $test_spotify_vbs) {
        ...
        $Shortcut.TargetPath = $source
        $Shortcut.Save()
        Write-Host "Скрипт очистки кэша удален" -ForegroundColor Green
        Write-Host "Установка завершена" -ForegroundColor Green
        exit
    }
    If (!($test_cache_spotify_ps -and $test_spotify_vbs)) {
